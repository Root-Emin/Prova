...
export function CriteriaAccordion({
  ...
  /** Only the org admin may override a score; the employee only reads it. */
  canOverride?: boolean
}) {
  return (
    <Accordion
              ...
              {criterion.override ? (
                <div className="rounded-md border border-line-strong bg-muted px-3 py-2">
                  <p className="prova-meta uppercase">
                    Yönetici ezdi · {criterion.override.overriddenBy}
                  </p>
                  <p className="mt-1 text-sm text-foreground">
                    Yeni durum: {rubricStatusView[criterion.override.newStatus].label}.{" "}
