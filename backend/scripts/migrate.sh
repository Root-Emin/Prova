...
case "${1:-}" in
    up)
        ...
        for f in "$MIGRATION_DIR"/0*.sql; do
            ...
            if [[ -n "$sql" ]]; then
                echo "$sql" | docker exec -i prova-postgres psql -U masterfabric -d masterfabric -q 2>/dev/null \
                    && echo "  ✓ $fname" \
    ...
    down)
        ...
        if [[ -n "$sql" ]]; then
            echo "$sql" | docker exec -i prova-postgres psql -U masterfabric -d masterfabric -q
    ...
    status)
        ...
        docker exec prova-postgres psql -U masterfabric -d masterfabric -c "
            SELECT 
